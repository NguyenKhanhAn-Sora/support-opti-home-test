...
Touch controllers (e.g. Cisco Touch 10)

They're the room's control panel, not a display.

External monitors or TVs attached to a room device

They're outputs of the room system, not separate signage endpoints.

IP phones, headsets, and other accessories

There's no display to show signage on.

Devices running in **Microsoft Teams Rooms (MTR) mode**

The display is managed by Microsoft, so OptiSigns can't control signage on it. A device needs to be in Cisco RoomOS mode to appear here.
...
Whether touch on a Board/Desk wakes the device or interacts with content

**Non-interactive (kiosk)** for lobby/info screens; **Interactive (touch)** for kiosk-style content

**Auto-refresh**

How often the device reloads the signage URL (`0 = never`)

30 minutes (default)

**Mute**

Whether videos play with sound

Off (default)

**Enable**

Master on/off for digital signage — disabling preserves the URL so you can re-enable later
